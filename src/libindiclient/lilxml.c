...
newLilXML ()
{
    LilXML *lp = (LilXML *) calloc(sizeof(LilXML), 1);
    initParser(lp);
    return (lp);
}
...
readXMLEle (LilXML *lp, int newc, char **errmsg)
{
    ...
    *errmsg = NULL;
    ...
    /* EOF? */
    if (newc == 0) {
        asprintf (errmsg, "Line %d: early XML EOF", lp->ln);
        initParser(lp);
        return (NULL);
    }
...
entityXML (char *s)
{
    static char *malbuf = NULL;
    int nmalbuf = 0;
    char *ns;
    char *ep;

    /* scan for each entity, if any */
    for (ns = s; (ep = strpbrk (ns, entities)) != NULL; ns = ep+1) {

        /* found another entity, copy preceding to malloced buffer */
        int nnew = ep - ns;			/* all but entity itself */
        malbuf = (char *)moremem (malbuf, nmalbuf + nnew + 10);
        memcpy (malbuf+nmalbuf, ns, nnew);
        nmalbuf += nnew;

        /* replace with entity encoding */
        switch (*ep) {
        case '&':  nmalbuf += sprintf (malbuf+nmalbuf, "&amp;"); break;
        case '<':  nmalbuf += sprintf (malbuf+nmalbuf, "&lt;"); break;
        case '>':  nmalbuf += sprintf (malbuf+nmalbuf, "&gt;"); break;
        case '\'': nmalbuf += sprintf (malbuf+nmalbuf, "&apos;"); break;
        case '"':  nmalbuf += sprintf (malbuf+nmalbuf, "&quot;"); break;
    ...
    } else { /* put remaining part of s into malbuf */
        int nleft = strlen (ns) + 1;		/* include \0 */
        malbuf = (char *)moremem (malbuf, nmalbuf + nleft);
        memcpy (malbuf+nmalbuf, ns, nleft);
    ...
    *errmsg = NULL;
    ...
    case LOOK4TAG:			/* looking for element tag */
        if (isTokenChar (1, c)) {
            ...
            asprintf (errmsg, "Line %d: Bogus tag char %c", lp->ln, c);
            return (-1);
        }
        break;
    ...
    case LOOK4ATTRN:		/* looking for attr name, > or / */
        if (c == '>')
            ...
            asprintf (errmsg, "Line %d: Bogus leading attr name char: %c", lp->ln, c);
            return (-1);
        }
        break;
    ...
    case SAWSLASH:			/* saw / in element opening */
        if (c == '>') {
            ...
            asprintf (errmsg, "Line %d: Bogus char %c before >", lp->ln, c);
            return (-1);
        }
        break;
    ...
    case INATTRN:			/* reading attr name */
        if (isTokenChar (0, c))
            ...
            asprintf (errmsg, "Line %d: Bogus attr name char: %c", lp->ln,c);
            return (-1);
        }
        break;
    ...
    case LOOK4ATTRV:		/* looking for attr value */
        if (c == '\'' || c == '"') {
            ...
            asprintf (errmsg, "Line %d: No value for attribute %s", lp->ln,
    ...
    case LOOK4CLOSETAG:		/* looking for closing tag after < */
        if (isTokenChar (1, c)) {
            ...
            asprintf (errmsg, "Line %d: Bogus preend tag char %c", lp->ln,c);
            return (-1);
        }
        break;
    ...
    case INCLOSETAG:		/* reading closing tag */
        if (isTokenChar(0, c))
            ...
            if (strcmp (lp->ce->tag.s, lp->endtag.s)) {
                asprintf (errmsg,"Line %d: closing tag %s does not match %s",
            ...
            asprintf (errmsg, "Line %d: Bogus end tag char %c", lp->ln, c);
            return (-1);
        }
        break;
    }
