...
void imf_set_dirty(struct image_file *imf, gboolean set_dirty)
{
    gboolean is_dirty = (imf->state_flags & IMG_STATE_DIRTY) != 0;

//    if (P_INT(FILE_SAVE_MEM)) {
//        if (is_dirty && ! set_dirty)
//            imf_release_frame(imf, "imf_set_dirty");
//        else if (set_dirty)
//            get_frame(imf->fr, "imf_set_dirty");
//    }
...
int imf_load_frame(struct image_file *imf)
{
    ...
    if (fr == NULL || reloaded > 0) {
        ...
        int ref_count = imf->fim->ref_count;
        *(imf->fim) = (struct wcs){ 0 }; // clear wcs
        imf->fim->ref_count = ref_count;
...
//        ofr_unlink_imf(imf->ofr);
    ...
    if (imf->fr) imf->fr = release_frame(imf->fr, msg);
...
//        if (! P_INT(FILE_SAVE_MEM)) // dont auto unload
...
d2_printf("reduce.ccd_reduce_imf setting background %.2f\n", imf->fr->stats.median);
    ...
    if ( (abort == 0) && (ccdr->op_flags & IMG_OP_BIAS) ) {
//        g_return_val_if_fail(ccdr->bias->fr != NULL, -1);
    ...
    if ( (abort == 0) && (ccdr->op_flags & IMG_OP_DARK) ) {
//        g_return_val_if_fail(ccdr->dark->fr != NULL, -1);
    ...
    if ( (abort == 0) && (ccdr->op_flags & IMG_OP_FLAT) ) {
//        g_return_val_if_fail(ccdr->flat->fr != NULL, -1);
    ...
    if ( (abort == 0) && (ccdr->op_flags & IMG_OP_BADPIX) ) {
//        g_return_val_if_fail(ccdr->bad_pix_map != NULL, -1);
    ...
    if ( (abort == 0) && (ccdr->op_flags & (IMG_OP_MUL | IMG_OP_ADD)) ) {
        ...
        if ( (ccdr->op_flags & (IMG_OP_MUL | IMG_OP_ADD)) != (imf->op_flags & (IMG_OP_MUL | IMG_OP_ADD)) ) {
            ...
            if (! (isnan(m) || isnan(a))) {
                if (ccdr->mul_before_add) {
                    op = " mul-add";
                    asprintf(&history, "'ARITHMETIC SCALE: P * %.3f + %.3f -> P'", m, a);
                } else {
                    op = " add-mul";
                    asprintf(&history, "'ARITHMETIC SCALE: (P + %.3f) * %.3f -> P'", a, m);
                }
            } else if (isnan(m)) {
                op = " add";
                asprintf(&history, "'ARITHMETIC SCALE: P + %.3f -> P'", a);
            } else if (isnan(a)) {
                op = " mul";
                asprintf(&history, "'ARITHMETIC SCALE: P * %.3f -> P'", m);
            }

            if (! (isnan(m) && isnan(a)))
                scale_shift_frame(imf->fr, isnan(m) ? 1.0 : m, isnan(a) ? 0 : a); // multiply then add

            if (history) fits_add_history(imf->fr, history), free(history);
    ...
    if ( (abort == 0) && (ccdr->op_flags & (IMG_OP_BG_ALIGN_ADD | IMG_OP_BG_ALIGN_MUL)) ) {
...
//                    scale_shift_frame (imf->fr, 1.0, ccdr->bg - imf->fr->stats.avg);
    ...
    if ( (abort == 0) && (ccdr->op_flags & IMG_OP_DEMOSAIC) ) {
        REPORT( " demosaic" )
    ...
    if ( ccdr->op_flags & IMG_OP_SUB_MASK && ! (imf->op_flags & IMG_OP_SUB_MASK) ) { // make a copy to work on
        imf_fr = imf->fr;
        ...
        imf_release_frame(imf, NULL);
    }

    if ( (abort == 0) && (ccdr->op_flags & IMG_OP_MEDIAN) ) {
        REPORT( " median" )
        ...
        if ( ! (imf->op_flags & IMG_OP_MEDIAN) ) {
            ...
            if (! median_with_star_exclusion(imf->fr, ccdr->medw)) // median filter with star exclusion
                ...
                fits_add_history_varg(imf->fr, "'MEDIAN FILTER %.0f'", ccdr->medw);
            ...
            if (abort && imf_fr) { // restore original imf_fr
                imf_release_frame(imf, "abort in IMG_OP_MEDIAN");

                imf->fr = imf_fr;
                frame_to_channel(imf->fr, ccdr->window, NULL);

//                imf_release_frame(imf, "after restore in IMG_OP_MEDIAN");
            }

        } else REPORT( " (already done)" )
    }

    if ( (abort == 0) && (ccdr->op_flags & IMG_OP_BLUR) ) {
        REPORT( " blur" )
        ...
        if ( ! (imf->op_flags & IMG_OP_BLUR) ) {
            ...
            if (res)
                ...
                fits_add_history_varg(imf->fr, "'GAUSSIAN BLUR (FWHM=%.1f)'", ccdr->blurv);
...
//                imf_release_frame(imf, NULL);
    ...
    if ( (abort == 0) && (ccdr->op_flags & IMG_OP_SUB_MASK) ) {
        REPORT( " sub_mask" )
...
//                imf_release_frame(imf, NULL);
                ...
                fits_add_history(imf->fr, "'SUB_MASK'");
    ...
    if ( (abort == 0) && (ccdr->op_flags & IMG_OP_ALIGN) ) {
        REPORT( " align" )
...
//        } else REPORT( " (already done)" )
    ...
    if ( (abort == 0) && (ccdr->op_flags & IMG_OP_WCS) ) {
        REPORT( " wcs" )
...
//                imf->state_flags &= IMG_STATE_SKIP; // clear flags but keep skip
    ...
    if (abort == 0) noise_to_fits_header(imf->fr);
...
//printf("reduce.reduce_frames\n");
    ...
    if (ret == 0) {
        ...
        while (gl != NULL) {
            ...
            if (ccdr->op_flags & IMG_OP_STACK) imf->state_flags |= IMG_STATE_STACK_PENDING;
...
//	n = i - 1;
    ...
    fits_add_history_varg(fr, "'AVERAGE STACK %d FRAMES'", i);
...
//	n = i - 1;
    ...
    fits_add_history_varg(fr, "'MEDIAN STACK of %d FRAMES'", i);
            ...
            progress_print_func progress, gpointer processing_dialog)
{
    ...
    fits_add_history_varg(fr, "'KAPPA-SIGMA STACK (s=%.1f) of %d FRAMES'", P_DBL(CCDRED_SIGMAS), i);
...
//	n = i - 1;
    ...
    fits_add_history_varg(fr, "'MEAN_MEDIAN STACK (s=%.1f) of %d FRAMES'", P_DBL(CCDRED_SIGMAS), i);
...
//            break;
    ...
    if (fr) {
        struct image_file *imf = add_image_file_to_list(imfl, fr, STACK_RESULT, IMG_STATE_LOADED | IMG_STATE_IN_MEMORY_ONLY | IMG_STATE_DIRTY);
        get_frame(fr, "stack_frames (stack result)");
    }
...
// set window wcs to alignref wcs (if validated)
...
// read wcs from alignment frame (if set)
        ...
        if (imf && imf->fim) {
            ...
            wcs->wcsset = WCS_HAVE_INITIAL(wcs) ? WCS_INITIAL : WCS_INVALID;
...
//    load_rcp_to_window (window, ccdr->recipe, NULL);
    ...
    if (ccdr->state_flags & IMG_STATE_REUSE_WCS) {
        ...
        wcs = window_get_wcs(ccdr->window);

        if (imf->fim->wcsset == WCS_VALID) { // or imf->fr.fim ?
            wcs_clone(wcs, imf->fim);
        } else {
            match_field_in_window_quiet(ccdr->window);
        }
	}
    ...
    for (;;) {
        ...
        struct gui_star_list *gsl = g_object_get_data (G_OBJECT(ccdr->window), "gui_star_list");
        result_ok = (gsl != NULL);
        if ( ! result_ok ) {
        ...
        struct stf *stf = run_phot (ccdr->window, wcs, gsl, imf->fr); // draws stars to fitted wcs
        result_ok = (stf != NULL);
        if ( ! result_ok ) {
...
// remove this bit? move to mband calls?
        if ( ccdr->multiband && ! (ccdr->state_flags & IMG_STATE_QUICKPHOT) ) { // add frame to ccdr->mbds
            ...
            if (ofr) {
                ...
                if (ret) {
                    printf ("%s\n", ret); fflush(NULL);
                    info_printf_sb2 (ccdr->window, ret);
                    free (ret);
                }
            }
...
//    release_frame(fr, "aphot_current_frame");
...
struct image_file * imf_new(struct ccd_frame *fr, char *filename)
{
    ...
    if (fr && filename) fr->name = strdup(filename);
...
void imf_release(struct image_file *imf)
{
	...
	if (imf->ref_count > 1) {
        printf("imf release %d '%s'\n", imf->ref_count, imf->filename); fflush(NULL);
    ...
    if (imf->fr) {
        // if FILE_SAVE_MEM, fr gets a ref when it becomes dirty
        if (imf->fr && P_INT(FILE_SAVE_MEM) && imf->state_flags & IMG_STATE_DIRTY)
            release_frame(imf->fr, "imf_release dirty");

        release_frame(imf->fr, "imf_release");
        if (imf->fr) imf->fr->imf = NULL;
...
//printf("reduce.add_image_file_to_list %s\n", filename);
    struct image_file *imf = imf_new(fr, filename);
    if (imf) {
        ...
        if (fr) {
            ...
            //        get_frame(imf->fr, "add_image_file_to_list"); // release in imfl_release
        }
    }
...
d2_printf("reduce.ccd_reduce_release %d\n", ccdr->ref_count);
    ...
    if (ccdr->multiband) dialog_delete_mbds(ccdr->multiband);
...
printf("imfl_release %d\n", imfl->ref_count); fflush(NULL);
    ...
    g_list_foreach(imfl->imlist, (GFunc)imf_release, NULL);
