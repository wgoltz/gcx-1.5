...
//    print_gui_stars(gsl->sl);
    ...
    while (sl != NULL) {
        ...
        if (STAR_OF_TYPE(gs, type_mask) && (gs->flags & flag_mask) == flag_mask) {
            remove_pair_from(gs);
...
struct gui_star *find_gs_by_cats_name(struct gui_star_list *gsl, char *name)
{
	...
	for (sl = gsl->sl; sl != NULL; sl = sl->next) {
        ...
        if ( ! STAR_OF_TYPE(gs, TYPE_CATREF) ) continue;
...
//        if (gs && (gs->flags & STAR_DELETED)) undelete_star(gs); // restore gs if it was deleted
        ...
        else if (STAR_OF_TYPE(gs, TYPE_PHOT)) continue; // why
...
//        if (gs && (gs->flags & STAR_DELETED)) undelete_star(gs); // restore gs if it was deleted
        ...
        else if (STAR_OF_TYPE(gs, TYPE_PHOT)) continue;
...
int update_gs_from_cats(GtkWidget *window, struct cat_star *cats)
{
    ...
    if (gs) {
        ...
        while (sl != NULL) {
            ...
            if (STAR_OF_TYPE(gs, TYPE_CATREF) && gs->s == cats) {
                found++;
